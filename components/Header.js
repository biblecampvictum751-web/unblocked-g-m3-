...
const Header = ({ onHomeClick, searchQuery, setSearchQuery }) => {
  return (
      ...
      <div className="container mx-auto px-4 max-w-7xl h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex-grow max-w-xl relative group">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all placeholder:text-slate-600"
          />
        </div>

        <div className="flex items-center gap-4 shrink-0">
          <button className="hidden md:flex bg-slate-900 border border-slate-800 hover:bg-slate-800 p-2 rounded-lg transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
